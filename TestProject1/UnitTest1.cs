...
namespace QQWRFO_HSZF_2024251.Tests
{
    ...
    public class PersonServiceTests
    {
        ...
        [Test]
        public void ExecuteOrder66_ShouldUpdateAllPaidPeopleStatusToOrdered()
        {
            // Arrange
            var people = new List<Person>
            {
                new Person { OrderStatus = Status.Paid },
                new Person { OrderStatus = Status.Paid }
            };

            _mockDataProvider.Setup(x => x.GetAllPeople()).Returns(people);
            _mockDataProvider.Setup(x => x.Modify(It.IsAny<Person>())).Verifiable();

            // Act
            _personService.ExecuteOrder66();

            // Assert
            _mockDataProvider.Verify(x => x.Modify(It.IsAny<Person>()), Times.Exactly(2));
        }
        [Test]
        public void CreatePerson_ShouldGenerateUniqueNeptunID_WhenPersonIsCreated()
        {
            // Arrange
            var name = "Alice Smith";
            var age = 22;
            var specialRequest = "None";
            var studentOrTeacher = "1";  // Student
            ...
            var existingNeptuns = new List<Person>
    {
        ...
        new Person { NeptunID = "ABCDE2" }
            ...
            // Check if the Neptun ID of the new person is unique (not in the existing list)
            var createdPerson = _mockDataProvider.Object.GetAllPeople().FirstOrDefault(p => p.Name == name);
            Assert.That(createdPerson, Is.Not.Null);
            Assert.That(existingNeptuns.Select(p => p.NeptunID), Does.Not.Contain(createdPerson.NeptunID));
        }
